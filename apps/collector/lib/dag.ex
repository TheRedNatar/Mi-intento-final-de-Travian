defmodule Collector.DAG do
  ...
  @spec run(root_folder :: String.t(), server_id :: TTypes.server_id(), target_date :: Date.t(), attemps :: pos_integer(), min :: pos_integer(), max :: pos_integer) :: :ok | {:error, any()}
  def run(root_folder, server_id, target_date, attemps, min, max) do
	...
	{:error, reason}
      :ok -> run_without_fetch(root_folder, server_id, target_date)
    end
  end
  ...
  @spec run_without_fetch(root_folder :: String.t(), server_id :: TTypes.server_id(), target_date :: Date.t()) :: :ok | {:error, any()}
  ...
  def reload(root_folder, server_id) do
    available_dates = Storage.list_dates(root_folder, server_id, Collector.RawSnapshot.options()) |> Enum.sort({:asc, Date})
