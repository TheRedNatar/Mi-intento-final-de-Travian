defmodule Collector.AggPlayers do
  ...
  @spec open(root_folder :: String.t(), server_id :: TTypes.server_id(), target_date :: Date.t()) :: {:ok, [t()]} | {:error, any()}
  ...
  @spec store(root_folder :: String.t(), server_id :: TTypes.server_id(), agg_players :: [t()], target_date :: Date.t()) :: :ok | {:error, any()}
  ...
  def run(root_folder, server_id, target_date) do
    with(
      ...
      {{:ok, prev_snapshot},{:ok, prev_agg_players}} <- open_option_prev_common_data(root_folder, server_id)
    ) do
	...
	_ ->
	  agg_players = process(target_dt, server_id, new_snapshot, prev_snapshot, prev_agg_players)
	  store(root_folder, server_id, agg_players, target_date)
      end
    end
    end
  ...
  defp open_option_prev_common_data(root_folder, server_id) do
    case Storage.list_dates(root_folder, server_id, Collector.snapshot_options()) do
      [_target_date] -> {{:ok, nil}, {:ok, nil}}
      available_snapshot_dates -> 
	[_target_date, prev_date | _] = Enum.sort(available_snapshot_dates, {:desc, Date})
	{Collector.open(root_folder, server_id, prev_date), open(root_folder, server_id, prev_date)}
    end
  end
