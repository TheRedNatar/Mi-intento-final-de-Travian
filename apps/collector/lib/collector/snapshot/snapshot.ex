defmodule Collector.Snapshot do
  ...
  @spec open_errors(root_folder :: String.t(), server_id :: TTypes.server_id(), target_date :: Date.t()) ::
          {:ok, [any()]} | {:error, any()}
  ...
  def run(root_folder, server_id, target_date) do
	    with(
	      {:a, {:ok, raw_snapshot}} <- {:a, Collector.RawSnapshot.open(root_folder, server_id, target_date)},
